...
class Model:
    ...
    def weight_variable(name, shape, stdev):
        initial = tf.random.truncated_normal(shape, stddev=stdev)
    ...
    def building_network(self, **kwargs):
        ...
        value_train = tf.nn.dropout(value_train, rate=(1-self.keep_prob))
    ...
    def _loss_func(self, name):
        t = tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.network_train, labels=self.y)
        self.loss_func = tf.reduce_mean(t, name=name)

    def _adam_optimize_operation(self, name):
        optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.learning_rate)
        self.trainer_func = optimizer.minimize(self.loss_func, name=name)

    def _efficacy_check(self, name):
        prediction = tf.nn.softmax(self.network_train)
        correct_pred = tf.equal(tf.argmax(prediction, 1), tf.argmax(self.y, 1))
        self.accuracy_func = tf.reduce_mean(tf.cast(correct_pred, tf.float32), name=name)

    def _predict_prep(self, name):
        self.predict_func = tf.nn.softmax(self.network_valid, name=name)

    def default_preproc(self):
        self._loss_func('loss')
        self._adam_optimize_operation('train')
        self._efficacy_check('accuracy')
        self._predict_prep('predict')
    ...
    def train(self, sess, ms):
        ...
        for step in range(1, self.steps_num + 1):
            ...
            sess.run(self.trainer_func, feed_dict={self.x: batch_x, self.y: batch_y,
                                                   self.keep_prob: self.drop_out})
            if step % self.display_step == 0 or step == 1:
                # Calculate batch loss and accuracy
                loss, acc = sess.run([self.loss_func, self.accuracy_func], feed_dict={self.x: batch_x,
                                                                                      self.y: batch_y,
                                                                                      self.keep_prob: 1.0})
    ...
    def predict(self, sess, ms):
        ...
        for idx_start in range(0, len(f_lists), self.batch_size):
            ...
            pred_seg = sess.run(self.predict_func, feed_dict={self.val_x: data_seg, self.keep_prob: 1.0})
            if first_pred:
